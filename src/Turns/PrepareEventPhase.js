...
import { PhaseState, CardState, BoxState } from "../Game/constants.js";
...
export default class PrepareEvent extends Phase{
    ...
    constructor(cardsInHandDeck, cardsInHandGrid, eventsPreparationDeck, eventsPrepationFGrid){
    ...
    execute() {
        if (this.#state === PhaseState.INIT) {
            ...
            this.#state = PhaseState.SELECT_HAND_CARD;
        } 
        
        if (this.#state === PhaseState.SELECT_HAND_CARD) {
            for (let i = 0; i < this.#cardsInHandDeck.length; i++) {
                if (this.#cardsInHandDeck[i].isClicked) { 
                    this.#selectedCard = this.#cardsInHandDeck[i];
                    this.#selectedCard.state = CardState.SELECTED;
                    this.#state = PhaseState.SELECT_TARGET_GRID;
                    return; 
                }
            }
        } 
        
        if (this.#state === PhaseState.SELECT_TARGET_GRID) {
